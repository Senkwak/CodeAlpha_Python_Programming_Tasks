...
def move_jpg_files(source_folder: str, target_folder: str) -> None:
    ...
    if not jpg_files:
        print("  ℹ️  No .jpg files found in the source folder.")
        return
    ...
    for name in jpg_files:
        ...
        if os.path.exists(dest):
            ...
            while os.path.exists(dest):
                ...
                counter += 1
            print(f"  ⚠️  Conflict: {name} → renamed to {os.path.basename(dest)}")
...
def extract_emails(input_file: str, output_file: str) -> None:
    ...
    if not found_emails:
        print("  ℹ️  No email addresses found in the file.")
        return
...
def main():
    ...
    # ── Quit ─────────────────────────────────────────────────
        ...
        print("\n  ⚠️  Invalid option. Please restart the program.\n")
